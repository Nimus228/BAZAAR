...
#include <QPixmap>
#include "viewadv.h"
profilExecutor::profilExecutor(user *thatUser, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::profilExecutor)
{
    ...
    while (!deal.atEnd()) {
        ...
        QStringList list = line.split(' ');
        if ((list[1].toInt() == thisUser->ID_user) and (list[3] == "принят")){
            adv.open(QIODevice::ReadOnly | QIODevice::Text);
            while (!adv.atEnd()){
                QString  lineAdv = adv.readLine();
                QStringList listAdv = lineAdv.split('~');
                if (listAdv[0].toInt() == list[2].toInt()){
                    img.open(QIODevice::ReadOnly | QIODevice::Text);
                    buttons.append(new QPushButton(container));
                    connect(buttons[countObj], &QPushButton::clicked, this, &profilExecutor::openObj);
                    buttons[countObj]->setObjectName(listAdv[0]);
                    ...
                    while (!img.atEnd()) {
                        ...
                        if (listAdv[4].toInt() == listImg[0].toInt()){
                            QImage imgg;
                            bool volid = imgg.load(listImg[1].remove("\n"));
                            if (volid){
                                imgg = imgg.scaledToWidth( buttons[countObj]->width(), Qt::SmoothTransformation);
                                buttons[countObj]->setIcon(QPixmap::fromImage(imgg));
...
profilExecutor::~profilExecutor()
{
    delete ui;
}

void profilExecutor::on_mini_clicked()
{
    if (ui->mini->isChecked()){
        miniMenu* menu = new miniMenu(thisUser);
        menu->show();
        connect(menu, &miniMenu::closeMain, this, &profilExecutor::close);
        connect(this, &profilExecutor::closeMini, menu, &miniMenu::close);
    }else{
        emit closeMini();
    }
...
void profilExecutor::on_editBut_clicked()
{
...
void profilExecutor::on_img_clicked()
{
    if (edit){
...
void profilExecutor::on_bazaar_clicked()
{
...
void profilExecutor::openObj()
{
    viewAdv *advWin = new viewAdv(thisUser, sender()->objectName());
    connect(advWin, &viewAdv::thisClose, this, &profilExecutor::activeWin);
    connect( this, &profilExecutor::advOp, advWin, &viewAdv::dealLook);
    emit advOp();
    advWin->show();
...
void profilExecutor::activeWin()
{
    this->setDisabled(false);
    profilExecutor *refresh = new profilExecutor(thisUser);
    refresh->show();
    this->close();
}
